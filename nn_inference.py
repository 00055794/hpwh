...
Pipeline:
  ...
  np.exp(ensemble_log) × price_index_current → nominal KZT/m² (current quarter)
...
Back-transform to nominal: np.exp(pred) × price_index_current   (Cell 184 methodology)
...
class NNInference:
    ...
    def __init__(self, model_dir: Path = MODEL_DIR):
        ...
        mape = self.metadata.get("test_mape_pct",
                                  self.metadata.get("ensemble", {}).get("test_mape_pct",
                                  self.metadata.get("MAPE", "?")))
    ...
    def predict_kzt(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        Step 2: multiply by price_index_current → nominal KZT/m² for today
        (Cell 184: "For nominal display: np.exp(pred) × price_index[current_quarter]")
        """
