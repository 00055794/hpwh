...
class StatLoader:
    ...
    def __init__(
        ...
        if lookup_path.exists():
            ...
            print(f"✔ StatLoader: {len(df)} centroids, {len(self._feature_cols)} feature cols, "
                  f"{len(self._region_lookup)} region-stat entries loaded")
        else:
            print(f"⚠ StatLoader: region_stat_lookup.json not found — using centroid fallback only")
